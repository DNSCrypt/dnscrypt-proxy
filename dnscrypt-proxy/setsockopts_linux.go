...
const (
	...
	// maxDNSPacketSize: stride hint for UDP GSO batch sends.
...
// propagating any error returned by fn.
func controlFd(c syscall.RawConn, fn func(int) error) error {
	var sockErr error
...
func setTCPSockOpts(fd int) error {
	...
	// Falls back to CUBIC silently if BBR is not compiled into the kernel.
