...
// Returns defaultPort when no port is present or is unparseable.
// IPv6 literals may be enclosed in brackets per RFC 3986 §3.2.2.
func ExtractHostAndPort(hostAndPort string, defaultPort int) (string, int) {
	h, portStr, err := net.SplitHostPort(hostAndPort)
	if err != nil {
...
// isIPAndPort returns nil when s is a valid "IP:port" string, or an error
// describing the problem. Used to validate bootstrap resolver addresses at
// startup.
func isIPAndPort(s string) error {
	...
	if net.ParseIP(host) == nil {
		return fmt.Errorf("host part %q is not a valid IP address", host)
	}
	port, err := strconv.Atoi(portStr)
	if err != nil || port <= 0 || port > 65535 {
		return fmt.Errorf("port part %q is not a valid port number", portStr)
	}
	return nil
}

// fqdn ensures host ends with a trailing dot, making it a fully-qualified
// domain name suitable for use in DNS query messages.
